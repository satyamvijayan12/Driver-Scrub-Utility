if exist "dist\Driver Scrub Utility.exe" (
    echo EXE already exists. Run dist\Driver Scrub Utility.exe
    pause
    exit /b 0
)@echo off
REM ============================================================
...
if exist "dist\Driver Scrub Utility.exe" (
    echo Existing executable found: dist\Driver Scrub Utility.exe
    echo.
    echo Press R to rebuild the EXE from source, or E to run the current build.
    set /p choice=Choice [R=Rebuild, E=Execute]:

    if /i "%choice%"=="E" (
        start "" "dist\Driver Scrub Utility.exe"
        exit /b 0
    )
    if /i not "%choice%"=="R" (
